...
class PositionalEncoding(nn.Module):
    ...
    def __init__(d_model: int, seq_len: int, dropout: float) -> None:
        super().__init__()
        self.d_model = d_model
        ...
        # Apply sine to even indices of the matrix
        pe[:, 0::2] = torch.sin(position * div_term) # sin(position * (10000 ** (2i / d_model))
        # Apply cosine to odd indices of the matrix
        pe[:, 1::2] = torch.cos(position * div_term) # cos(position * (10000 ** (2i / d_model))
        # Add a batch dimension to the positional encoding
        pe = pe.unsqueeze(0) # (1, seq_len, d_model)
        # Register the positional encoding as a buffer to keep tensor in the file of a model but not as a learned parameter
        self.register_buffer('pe', pe)

    def forward(self, x):
        # Add the positional encoding to every word inside the sentence
        x = x + (self.pe[:, :x.shape[1], :]).required_grad(False) # (Batch_size, seq_len, d_model)
...
class FeedForwardBlock(nn.Module):
    ...
    def __init__(self, d_model: int, d_ff: int, dropout: float = 0.1) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(d_model, d_ff) # W1 & B1 # by deafault bias is true
        self.dropout = nn.Dropout(dropout)
        self.linear_2 = nn.Linear(d_ff, d_model) # W2 & B2 # by deafault bias is true

    def forward(self, x):
        # (Batch_size, seq_len, d_model) -> (Batch_size, seq_len, d_ff) -> (Batch_size, seq_len, d_model)
        # Inout sentence -> Linear1 -> Linear2
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, d_k, mask=None, dropout=nn.Dropout):
        d_k = query.shape[-1] # last dimension
        # Applying formula from paper
        # (Batch_size, h, seq_len, d_k) -> (Batch_size, h, seq_len, seq_len
        attention_scores = (query @ key.transpose(-2, -1)) / math.sqrt(d_k)
        # in the transpose(-2, -1), the last 2 dimensions are swapped (seq_len, d_k) -> (d_k, seq_len)
        ...
        # tuple because it is going to next layer, attention scores is for visualization
    ...
    def forward(self, q, k, v, mask):
        ...
        # output, attention scores (output of softmax)
...
class ProjecttionLayer(nn.Module):

    def __initII(self, d_model: int, vocab_size: int) -> None:
        super().__init__()
